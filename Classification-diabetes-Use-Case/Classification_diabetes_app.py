# -*- coding: utf-8 -*-
"""
Created on Mon Dec 18 14:21:03 2023
...
html_temp = """
    ...
    <h1 style="color: white; text-align:center">Machine Learning Dashboard</h1>
    </div>
    <p style="font-size: 20px; font-weight: bold; text-align:center">Diabetes Use Case: Classification</p>
    """
...
class DataPreprocessing:
    ...
    def __init__(self, file):
        self.data = pd.read_csv(file.open("diabetes.csv"))
...
class ModelBuilding:
    ...
    def add_parameter_ui(self, clf_name):
        ...
        if clf_name == "Logistic Regression":
            ...
            params["max_features"] = st.sidebar.selectbox('Max Features', ['sqrt', 'log2'], index=0)
            ...
            params["criterion"] = st.sidebar.selectbox('Criterion', ['squared_error', 'mae'], index=0)
            params["max_depth"] = st.sidebar.slider('Max Depth', 1, 100, None, 1)
            params["min_samples_split"] = st.sidebar.slider('Min Samples Split', 2, 20, 2, 1)
            params["min_samples_leaf"] = st.sidebar.slider('Min Samples Leaf', 1, 20, 1, 1)
            params["max_features"] = st.sidebar.selectbox('Max Features', ['sqrt', 'log2'], index=0)
    ...
    def get_algorithm(self, clf_name, params):
        ...
        if clf_name == "LightGBM Regression":
            regressor = LGBMRegressor(**params)  # Instantiate LightGBM regressor with parameters
            return regressor
        elif clf_name == "Perceptron":
            clf = Perceptron(
                alpha=params["alpha"],
...
def app(): 
    ...
    if file is not None:
        with st.sidebar.expander('preprdocessing'):
            ...
            target_variable = st.sidebar.text_input("Target variable",value="Outcome")
        ...
        if ml_problem == 'None':
            ...
            params = Model_instance.add_parameter_ui(problem_type)
            regressor = Model_instance.get_algorithm(problem_type, params)
            ...
            with training_expander:
                ...
                """
                model_download = st.button("Download Trained Model")
                if model_download:
                    # Serialize the trained model and download it
                    filename = f"{classifier}.pkl"
                    with open(filename, 'wb') as file:
                        pickle.dump(regressor, file)
                    st.write("Trained model has been downloaded.")
                    st.markdown(f"Download [trained_model.pkl](./{filename})")
                """
        ...
        #model_file = pd.read_pickle("C:/Users/dell/OneDrive - Université Mohammed VI Polytechnique/Documents/ENSAM/Projects/Machine-Learning-Dashboard-main/Machine-Learning-Dashboard-main/Classification diabetes Use Case/{classifier}.pkl")
